...
<Tip>
Gesture names like `mini_wave_hello` and `talk_head_nod_subtle` are the action identifiers you pass to `VideoControl(action=...)` or the RPC `trigger_dynamics` method. See [Deployment](/guides/deployment) for integration examples.
</Tip>
