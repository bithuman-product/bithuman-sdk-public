...
<Tip>
**Want a working app right now?** Reference apps (Mac, iPad, iPhone) live in the private [bithuman-apps](https://github.com/bithuman-product/bithuman-apps) repo. They consume the SDK via the published SwiftPM binary package — the same way any external developer would. Walk through this page if you're integrating the SDK into an existing project; request access to bithuman-apps if you want to read finished code.
</Tip>
...
- **A target device that meets the
  [hardware floor](/swift-sdk/overview#hardware-floor)** — under-spec devices
...
<Note>
The `bitHumanKit` Swift Package is the **legacy** Swift binary distribution (Mac/iPad/iPhone voice-chat + Expression engine), distributed from `bithuman-sdk-public` and still pinned to `0.8.1`. New cross-platform integrations should target the unified `Bithuman` SwiftPM product (`from: "1.17.0"`) — see [Streaming API](/swift-sdk/streaming) and [Install](/getting-started/install#swift--swiftpm). The two tracks coexist; `bitHumanKit` covers the high-level voice-chat orchestrator with on-device LLM/TTS, `Bithuman` exposes the streaming engine binding bit-exact across mac/iOS/Android.
</Note>
...
> API key first**: [https://www.bithuman.ai](https://www.bithuman.ai) → Developer → API Keys.
...
- **Reference apps** — Mac, iPad, and iPhone apps in the private [bithuman-apps](https://github.com/bithuman-product/bithuman-apps) repo. They consume the SDK via the published SwiftPM binary package. Drag-drop face swap, PiP, Stage Manager widget, all wired up.
- **[Platform-specific guides](/swift-sdk/macos)** — entitlements, distribution, and host-specific patterns for [macOS](/swift-sdk/macos) and [iOS / iPadOS](/swift-sdk/ios).
- **[CLI](/swift-sdk/cli)** — `brew install bithuman-cli` for a no-code Mac terminal app built on the same SDK.
- **[Troubleshooting](/swift-sdk/troubleshooting)** — every error pattern with the fix.
