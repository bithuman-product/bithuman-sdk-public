...
```bash
pip install 'bithuman==2.1.0'
```

**Python 3.11** is the only version with wheels on the current
2.1.0 release. Platforms: macOS arm64, Linux x86_64, Linux aarch64.
(Windows wheels were last published with 1.9.0 and are not yet
back in the 2.x matrix.)

<Warning>
If you're on a different Python version, install will fail with
`ERROR: Could not find a version that satisfies the requirement
bithuman`. Use `pyenv` or a Python 3.11 venv:

```bash
pyenv install 3.11.10           # one-time
pyenv shell 3.11.10
python -m venv .venv && source .venv/bin/activate
pip install 'bithuman==2.1.0'
```

Broader wheel coverage (3.10, 3.12, 3.13, 3.14 across macOS/Linux,
plus Windows) is on the roadmap. Watch the
[changelog](/changelog).
</Warning>
...
- **Python 3.11** (only version with current 2.1.0 wheels; see
