...
| Approach | Model | Best For | Model Files | Compute | Internet |
|----------|-------|----------|-------------|---------|----------|
| [Cloud Plugin](#cloud-plugin) | Essence or Expression | Getting started, web apps | No | bitHuman cloud | Per turn |
| [Self-Hosted CPU](#self-hosted-cpu) | Essence | Privacy, edge devices, kiosks | Yes (`.imx`) | Your CPU | Auth only |
| [Self-Hosted GPU](#self-hosted-gpu) | Expression | Dynamic faces, custom images | No (uses images) | Your NVIDIA GPU | Auth only |
| [On-Device Apple Silicon](#on-device-apple-silicon) | Expression | Native consumer apps, offline-first | No (universal weights) | User's M3+ Mac / M4+ iPad / A18 Pro+ iPhone | Heartbeat only |
