...
`bithuman[local]` is an opt-in extra that swaps the cloud conversation
...
Ships in **bithuman 2.2.1+** for macOS arm64, Linux x86_64, Linux aarch64,
Python 3.10–3.14.
...
└─ bithuman.serve_brain                  (Python livekit-agents worker)
...
(`bithuman.serve_brain`), and the **same `AvatarSession`** plugin. One
...
- **Avatar generation** — you still need an `.imx` file. Generate one on
  [bithuman.ai](https://www.bithuman.ai/#explore) (free tier), or use
  the bundled sample (`~/.cache/bithuman/models/sample-avatar.imx`).
...
installed `bithuman` without the `[local]` extra. Re-install:
`pip install 'bithuman[local]'`.
