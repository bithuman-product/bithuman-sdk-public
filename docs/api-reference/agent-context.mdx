...
<Note>
This API is for agents created on the bitHuman platform with an active session. It's not for local SDK agents.

**Need credentials?** Get your agent code by [generating an agent](/api-reference/agent-generation) or from your [Library](https://www.bithuman.ai/#library). Get your API secret from [Developer → API Keys](https://www.bithuman.ai/#developer).
</Note>
