...
| `INSUFFICIENT_CREDITS` | 402 | Insufficient credits | Top up credits at [www.bithuman.ai](https://www.bithuman.ai). |
...
if resp.status_code == 200:
    data = resp.json()["data"]
    print(f"Agent {data['agent_id']} is generating...")

elif resp.status_code == 401:
    print("Invalid API secret. Check BITHUMAN_API_SECRET.")

elif resp.status_code == 429:
    retry_after = resp.json()["error"].get("retry_after", 5)
