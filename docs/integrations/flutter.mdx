...
The plugin ships with a cross-platform example app demonstrating settings, prompt hot-reload, and the full Essence pipeline. For Expression on Apple Silicon (Mac and iPad), pair the plugin with the native Swift [Expression on Swift](/swift-sdk/essence) guide — the Flutter plugin itself covers the Essence cross-platform path today.
