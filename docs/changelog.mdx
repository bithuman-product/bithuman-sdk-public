...
- **PyPI wheel split.** `pip install bithuman` is now the Python SDK
  **library only** (~5 MB) — `from bithuman import AsyncBithuman`
  still works. The CLI moved to the sibling
  [`bithuman-cli`](https://pypi.org/project/bithuman-cli/) wheel;
  Homebrew (`brew install bithuman-product/bithuman/bithuman-cli`) and the
  universal `curl … | sh` installer are unchanged.
- **Layered architecture.** Engine, SDKs, and apps are now cleanly
  separated across two private repos and two public ones:
  [`bithuman-sdk`](https://github.com/bithuman-product/bithuman-sdk)
  holds the `libessence` engine + Python / Swift / Kotlin / Rust SDKs +
  the `parity/` contract tests;
  `bithuman-apps` *(private)* holds the CLI, the Flutter plugin, and
  the Expression reference apps;
  [`bithuman-sdk-public`](https://github.com/bithuman-product/bithuman-sdk-public)
  is this docs source + examples;
  [`homebrew-bithuman`](https://github.com/bithuman-product/homebrew-bithuman)
  is the tap + universal installer. See
...
- **No external API breaks.** Python (`from bithuman import
  ...
  install-time only: `pip install bithuman-cli` (or use Homebrew) to
  keep the `bithuman` console-script.
- **Engine ABI** bumps to `v7` — adds
