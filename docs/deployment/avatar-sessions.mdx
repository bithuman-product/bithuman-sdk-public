---
title: "Run avatar sessions on cloud, CPU, or GPU"
sidebarTitle: "Avatar sessions"
description: "Bring a bitHuman avatar into a LiveKit room. Cloud plugin, self-hosted CPU, self-hosted GPU, or on-device — with a working code example for each."
icon: "book-open"
---
...
The cloud plugin runs the avatar on bitHuman's servers. Provide an agent code and API secret — no model files, no GPU.
...
Open [agents-playground.livekit.io](https://agents-playground.livekit.io) to connect and talk to your avatar.

### What happens under the hood

When `avatar.start()` and `session.start()` fire:

1. The plugin requests a session from bitHuman's cloud API.
2. A cloud avatar worker picks it up and downloads the model (cached afterwards).
3. The worker joins your LiveKit room as a participant named `bithuman-avatar-agent`.
4. As your agent produces TTS audio, the worker generates animated video frames.
5. Video publishes to the room — users see the avatar speaking.

<Tip>
**Essence or Expression?** The cloud plugin uses **Essence** (pre-built `.imx`) by default. Pass `model="expression"` to use **Expression** (any face image at runtime).
</Tip>

### Use Expression with a custom face image
...
Run the avatar entirely on your own machine using a downloaded `.imx` model file. Right for privacy and offline use.
...
### How self-hosted CPU differs from cloud
...
<Note>
For full GPU container setup, see [Self-hosted GPU](/deployment/self-hosted-gpu).
</Note>
...
- **Expression on-device** — `bitHumanKit` SwiftPM package on Mac M3+ /
  iPad Pro M4+ via MLX. iPhone runs Essence only — Expression needs
  iPad Pro M4+ or Mac M3+.
...
| **iPhone** | Essence only — Expression needs iPad Pro M4+ or Mac M3+. See [Essence on Swift](/swift-sdk/essence). |
...
## Add gestures, dynamics, and REST control
