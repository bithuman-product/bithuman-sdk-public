...
<Tabs>
  <Tab title="CLI (no code)">
    macOS Homebrew, the universal installer, or the `bithuman-cli`
    PyPI sibling wheel — all ship the same Rust binary:

    ```bash
    brew install bithuman-product/bithuman/bithuman    # macOS
    # or:
    pip install bithuman-cli                            # Python-only env
    ...
    `pip install bithuman` (no `-cli`) is the **library** —
  ...
  <Tab title="Python">
    ...
    LiveKit plugin glue ship in this wheel; the `bithuman` CLI is
    a separate package (`pip install bithuman-cli`).

    For a **fully on-device brain** (no OpenAI key, no outbound
    network) on top of the CLI, add the `[local]` extra and flip one
    env var:
