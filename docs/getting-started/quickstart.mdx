...
# x86_64/aarch64; pin the version below to match what we've tested.
...
<Warning>
**Python 3.11 only on the pip path right now.** 2.1.0 ships
`cp311` wheels only (macOS arm64, Linux x86_64, Linux aarch64). If
you're on a different Python version, either pyenv your project to
3.11 (`pyenv install 3.11.10 && pyenv shell 3.11.10`) or use the
Homebrew CLI instead — it has no Python dependency. Broader wheel
