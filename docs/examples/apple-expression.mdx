...
<Note>
This is the **Python SDK** path to **Expression on macOS** — `pip install bithuman` plus a bundled Swift daemon, called from Python.

- For a **native Swift Expression app** (SwiftUI / AppKit / UIKit) on Mac M3+ or iPad Pro M4+, see the [Swift SDK quickstart](/swift-sdk/quickstart) and the [`expression/mac/`](https://github.com/bithuman-product/bithuman-apps/tree/main/expression/mac) / [`expression/ipad/`](https://github.com/bithuman-product/bithuman-apps/tree/main/expression/ipad) reference apps.
- For **Essence on-device** (which also runs on iPhone, Android, and the iPad Pro Air), see [Essence on Swift](/swift-sdk/essence), [Kotlin SDK quickstart](/kotlin-sdk/quickstart), or the canonical cross-platform [Flutter `bithuman` plugin](/integrations/flutter).

Both Essence and Expression are production-ready; pick by use case (custom portraits per session vs. kiosk-style branded character) rather than by "default."
</Note>
...
<Accordion title="Clone the full example repo">
  ...
  cd bithuman-sdk-public/Examples/python/local-expression-mac
