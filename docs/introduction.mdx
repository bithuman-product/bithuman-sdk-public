...
description: "Turn any face into a lifelike talking avatar. Audio in, animated video out — at 25 FPS. Python SDK, Swift SDK (on-device macOS M3+), REST API, LiveKit plugin, and self-hosted GPU containers."
keywords:
  - bitHuman
...
- **Self-Hosted GPU (Linux + NVIDIA)** — any face image, real-time lip-sync from the Expression model on your own GPU.
- **[On-Device macOS](/examples/apple-expression)** — Expression avatar running natively on Apple Silicon (M3+) via the bundled Swift runtime. No GPU, no cloud, no Docker. The same pipeline also powers [bitHuman Halo](/examples/halo-macos), our free desktop companion app.
...
| **macOS (Apple Silicon)** | Full Support | Expression (on-device) requires M3+; Essence runs on any Apple Silicon |
| **Windows (x86_64)** | Full Support | Native wheels for Python 3.9–3.14, or WSL |
