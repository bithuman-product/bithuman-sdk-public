...
def get_custom_gpu_endpoint() -> str:
    """
    Get the custom GPU endpoint URL from environment.

    Supported endpoint formats:
    - Self-hosted: http://gpu-avatar-worker:8089/launch (Docker internal)
    - Self-hosted: https://your-domain.com/launch (public URL)
...
def load_avatar_image() -> Image.Image | str | None:
    """
    Load avatar image from BITHUMAN_AVATAR_IMAGE environment variable.

    The environment variable can be:
    - A URL (https://...)
    - A local file path
    ...
    if not avatar_source:
        logger.info("No BITHUMAN_AVATAR_IMAGE specified, using worker default")
        return None
