...
<Warning>
**Not on pub.dev yet.** The plain `bithuman: ^1.16.0` snippet you may
see in older docs will fail `flutter pub get` with "package not found".
The plugin is currently distributed as a git dependency only.
**Request the git URL on [Discord](https://discord.gg/ES953n7bPA)**
(we gate it during early access); the pub.dev publish announcement
will land in the [changelog](https://docs.bithuman.ai/changelog) and
on Discord.
</Warning>

## Install

Once you have the git URL from Discord, pin it in `pubspec.yaml`:

```yaml
# pubspec.yaml — replace <git-url-from-discord> with the URL we share with you
dependencies:
  bithuman:
    git:
      url: <git-url-from-discord>
      ref: v1.16.0    # pin to a release tag (don't track main)
```
...
<Tip>
After pub.dev publish, this will become a one-line
`bithuman: ^X.Y.Z` and `flutter pub get` will resolve directly. The
public API will be source-compatible across the transition.
</Tip>
