...
This page covers the middle path — Expression on an M3+ Mac.
...
<Steps>
  ...
  <Step title="Download a packed Expression .imx">
    Grab one from your bitHuman dashboard, or pack your own from raw weights with `bithuman pack` (see [Packing at different resolutions](https://github.com/bithuman-product/bithuman-python-sdk#packing-at-different-resolutions) in the SDK README).
  </Step>
  ...
  <Step title="Run the CLI demo">
    ...
    <Check>Expect **`✓ Wrote demo.mp4 — 393 frames @ 25 FPS`** and a lip-synced video of the default face talking for 15 s.</Check>
  </Step>

  <Step title="Try a different face (optional)">
    Pass a portrait image or URL — the SDK encodes it on load (~300 ms):
    ...
    Portraits are cached under `~/.cache/bithuman/identities/` after the first download.
  </Step>
...
<CardGroup cols={2}>
  <Card title="Python (recommended)" icon="python" href="https://github.com/bithuman-product/bithuman-examples/tree/main/apple-expression">
    `pip install bithuman` + `bithuman demo`. Auto-dispatches to the Swift daemon on macOS M3+, or to the in-process Essence runtime everywhere else. What this page walks through.
  </Card>
